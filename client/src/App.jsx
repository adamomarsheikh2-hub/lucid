  ...
  return (
    <div style={{ minHeight:'100vh', fontFamily:FONT, color:t.text, background:t.pageSolid, boxSizing:'border-box' }}>
      ...
      <header style={{ borderBottom:`1px solid ${t.hairline}`, background:t.headerBg, backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)', position:'sticky', top:0, zIndex:20 }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding: mobile ? '10px 14px' : '12px 24px' }}>

          {/* Logo */}
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:34, height:34, borderRadius:10, background:t.accentGrad, display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, boxShadow:`0 8px 22px -6px rgba(${t.accentGlow},0.75), inset 0 1px 0 rgba(255,255,255,0.45)` }}>
              <span style={{ color:'#fff', fontSize:15, fontWeight:800 }}>L</span>
            </div>
            <div>
              <span style={{ fontSize: mobile ? 14 : 16, fontWeight:650, color:t.text }}>LucidFlex</span>
              {!mobile && <span style={{ fontSize:14, color:t.faint, fontWeight:400 }}> · {acc.name || '…'}</span>}
            </div>
          </div>
          ...
          <div style={{ display:'flex', alignItems:'center', gap: mobile ? 8 : 16 }}>
            <div style={{ display:'flex', alignItems:'center', padding:3, borderRadius:13, background:'rgba(255,255,255,0.045)', border:`1px solid rgba(255,255,255,0.07)`, backdropFilter:'blur(14px)' }}>
              {allUsers.map(u => {
                ...
                return (
                  <button key={u.id} onClick={() => onSwitch(u)} style={{ padding: mobile ? '6px 12px' : '7px 17px', borderRadius:10, border:'none', cursor:'pointer', fontFamily:FONT, fontSize: mobile ? 12 : 13, fontWeight:600, transition:'all 0.15s', color:active?t.text:t.muted, background:active?'linear-gradient(180deg,rgba(255,255,255,0.18),rgba(255,255,255,0.06))':'transparent', boxShadow:active?'inset 0 1px 0 rgba(255,255,255,0.28), 0 2px 8px -2px rgba(0,0,0,0.5)':'none' }}>{u.name}</button>
                )
              })}
            </div>
            <button onClick={() => setDark(d => !d)} style={{ width:32, height:32, borderRadius:9, border:`1px solid ${t.hairline}`, background:t.surface, color:t.muted, fontSize:14, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', transition:'all 0.15s' }} onMouseEnter={e => e.currentTarget.style.color=t.text} onMouseLeave={e => e.currentTarget.style.color=t.muted}>{dark ? '☀' : '☾'}</button>
            {!mobile && <div style={{ width:1, height:22, background:'rgba(255,255,255,0.10)' }}/>}
            {!confirmReset ? (
              <button onClick={() => setConfirmReset(true)} style={{ height:32, padding:'0 10px', borderRadius:8, border:'none', background:'transparent', color:t.muted, fontSize: mobile ? 12 : 13, cursor:'pointer', fontFamily:FONT, transition:'color 0.15s' }} onMouseEnter={e => e.currentTarget.style.color=t.text} onMouseLeave={e => e.currentTarget.style.color=t.muted}>Reset</button>
            ) : (
              <>
                <button onClick={resetData} style={{ height:32, padding:'0 10px', borderRadius:8, border:'none', background:t.redDim, color:t.red, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:FONT }}>Confirm</button>
                <button onClick={() => setConfirmReset(false)} style={{ height:32, padding:'0 8px', borderRadius:8, border:'none', background:'transparent', color:t.muted, fontSize:12, cursor:'pointer', fontFamily:FONT }}>Cancel</button>
              </>
            )}
          </div>
        </div>

        {/* Mobile: scrollable account pills row */}
        {mobile && (
          <div style={{ padding:'0 12px 10px', display:'flex', gap:6, overflowX:'auto', WebkitOverflowScrolling:'touch' }}>
            {accountList.map(a => {
              const active = a.id === activeAccountId
              return (
                <button key={a.id} onClick={() => switchAccount(a.id)} style={{ flexShrink:0, padding:'5px 14px', borderRadius:20, border:`1px solid ${active ? `rgba(${t.accentGlow},0.4)` : t.hairline}`, background: active ? `rgba(${t.accentGlow},0.12)` : 'transparent', color: active ? t.accent : t.muted, fontFamily:FONT, fontSize:12, fontWeight:600, cursor:'pointer', whiteSpace:'nowrap', transition:'all 0.15s' }}>{a.name}</button>
              )
            })}
            <button onClick={() => setShowNewAccount(true)} style={{ flexShrink:0, padding:'5px 14px', borderRadius:20, border:`1px dashed ${t.hairline}`, background:'transparent', color:t.muted, fontFamily:FONT, fontSize:12, fontWeight:500, cursor:'pointer', whiteSpace:'nowrap' }}>+ New</button>
          </div>
        )}
      </header>

      {/* ── Left sidebar (fixed, desktop only) ── */}
      {!mobile && <div style={{ position:'fixed', left:0, top:63, bottom:0, width:210, zIndex:18, overflowY:'auto', borderRight:`1px solid ${t.hairline}`, background:t.headerBg, backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)' }}>
        <div style={{ padding:'20px 0 20px' }}>
          <div style={{ padding:'0 16px 10px', fontSize:10, color:t.muted, fontWeight:700, letterSpacing:'0.14em', textTransform:'uppercase' }}>Eval Accounts</div>
          {accountList.map(a => {
            const active = a.id === activeAccountId
            return (
              <div key={a.id} style={{ display:'flex', alignItems:'center' }}>
                <button onClick={() => switchAccount(a.id)} style={{ flex:1, display:'flex', alignItems:'center', gap:9, padding:'9px 16px', background: active ? `rgba(${t.accentGlow},0.12)` : 'transparent', border:'none', cursor:'pointer', fontFamily:FONT, textAlign:'left', minWidth:0, transition:'background 0.12s' }}
                  ...
                  <div style={{ minWidth:0, flex:1 }}>
                    ...
                    <div style={{ fontSize:10, color:t.muted, fontVariantNumeric:'tabular-nums' }}>${(a.size/1000).toFixed(0)}K</div>
                  </div>
                </button>
          ...
          <div style={{ height:1, background:t.hairline, margin:'10px 12px' }}/>
          <button onClick={() => setShowNewAccount(true)} style={{ width:'100%', display:'flex', alignItems:'center', gap:8, padding:'9px 16px', border:'none', background:'transparent', color:t.muted, fontSize:12, fontWeight:600, cursor:'pointer', fontFamily:FONT, transition:'all 0.12s', textAlign:'left' }}
      ...
      <div style={{ marginLeft: mobile ? 0 : 210, marginRight: mobile ? 0 : (newsOpen ? 224 : 40), position:'relative', zIndex:1 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:18 }}>
            <div>
              ...
              <div style={{ display:'flex', alignItems:'baseline', gap:14 }}>
                <span style={{ fontSize: mobile ? 40 : 62, fontWeight:760, letterSpacing:'-0.03em', lineHeight:1, fontVariantNumeric:'tabular-nums', color:t.textStrong }}>
                  ${displayBalance.toLocaleString(undefined, { maximumFractionDigits:0 })}
                </span>
                <span style={{ fontSize: mobile ? 17 : 22, fontWeight:700, color:totalPnl>=0?t.green:t.red, letterSpacing:-0.3 }}>{sfmt(totalPnl)}</span>
              </div>
            </div>
        ...
        {/* ── Limits ── */}
        <div style={{ ...glass(t, dark), display:'grid', gridTemplateColumns: mobile ? '1fr' : '1fr 1px 1fr', marginBottom: mobile ? 16 : 22 }}>
          <Sheen/>
          <div style={{ position:'relative', zIndex:1, padding: mobile ? '20px 20px' : '26px 30px' }}>
          ...
          {!mobile && <div style={{ background:'rgba(255,255,255,0.07)', zIndex:1 }}/>}
          <div style={{ position:'relative', zIndex:1, padding: mobile ? '20px 20px' : '26px 30px', borderTop: mobile ? `1px solid ${t.hairline}` : 'none' }}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
        ...
        {/* ── Equity Curve + Stats ── */}
        <div style={{ display:'grid', gridTemplateColumns: mobile ? '1fr' : 'minmax(0,1.62fr) minmax(220px,1fr)', gap: mobile ? 16 : 22, marginBottom: mobile ? 16 : 22 }}>
        ...
        {/* Mobile: collapsible news */}
        {mobile && (
          <div style={{ marginTop:16 }}>
            <button onClick={() => setNewsOpen(o => !o)} style={{ width:'100%', display:'flex', alignItems:'center', justifyContent:'space-between', padding:'14px 18px', borderRadius:14, border:`1px solid ${t.hairline}`, background:t.surface, color:t.text, fontFamily:FONT, fontSize:14, fontWeight:600, cursor:'pointer', backdropFilter:'blur(20px)', WebkitBackdropFilter:'blur(20px)' }}>
              <span>US Economic Calendar</span>
              <span style={{ color:t.muted, fontSize:16, display:'inline-block', transition:'transform 0.2s', transform: newsOpen ? 'rotate(180deg)' : 'rotate(0deg)' }}>▾</span>
            </button>
            {newsOpen && (
        ...
        </div>{/* end zoom/padding */}
      </div>{/* end margin offsets */}
    </div>
  )
}
